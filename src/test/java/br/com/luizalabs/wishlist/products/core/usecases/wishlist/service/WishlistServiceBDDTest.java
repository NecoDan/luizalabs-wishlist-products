...
public class WishlistServiceBDDTest {
    ...
    public void setUp() {
        ...
        BDDMockito.when(jpaWishlistRepository.findAll())
                .thenReturn(List.of(wishlist));

        BDDMockito.when(jpaWishlistRepository.findById(anyString()))
                .thenReturn(Optional.of(wishlist));

        BDDMockito.when(jpaWishlistRepository.save(WishlistCreator.createdValidWishListToBeSaved(LIST_NAME)))
                .thenReturn(wishlist);

        BDDMockito.doNothing().when(jpaWishlistRepository).deleteById(anyString());

        BDDMockito.when(jpaWishlistRepository.findAllByClientId(anyString()))
                .thenReturn(List.of(wishlist));

        BDDMockito.when(jpaWishlistRepository.findAllWishListClientIdAndProductId(anyString(), anyString()))
                .thenReturn(List.of(wishlist));
    ...
    void saveCreatesWishlistWhenSuccessful() {
        ...
        BDDMockito.when(jpaWishlistRepository.save(wishlistSaved)).thenReturn(wishlist);
